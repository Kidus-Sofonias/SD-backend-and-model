# File role: Phase 4 — recalculate historical trips with the v2 scoring methodology.
# Re-runs the current ML/scoring pipeline over every stored completed trip and
# updates the persisted score, risk fields, breakdown, and driving events.
#
# Safety behavior:
# - Trips whose raw samples were deleted (raw_deleted=True) CANNOT be recomputed;
#   they are skipped and reported (their stored score is left untouched).
# - Trips already processed under scoring_version "v2" are skipped by default
#   (pass --include-v2 to force a full re-run). This makes the script resumable
#   and idempotent.
# - --dry-run computes the new rule score + event set WITHOUT writing anything.
#
# Usage:
#   python -m scripts.recalculate_trips_phase4 --dry-run
#   python -m scripts.recalculate_trips_phase4
#   python -m scripts.recalculate_trips_phase4 --trip-id <id>
#   python -m scripts.recalculate_trips_phase4 --user-id <id>
#   python -m scripts.recalculate_trips_phase4 --include-v2
#
# The JSON report is written to artifacts/reports/phase4_recalc_report.json.
...
REPORT_PATH = BACKEND_ROOT / "artifacts" / "reports" / "phase4_recalc_report.json"


def _is_scoring_version_v2(breakdown_raw: str | None) -> bool:
    ...
    if not isinstance(breakdown, dict):
        return False
    if breakdown.get("scoring_version") == "v2":
        return True
    nested = breakdown.get("rule_breakdown")
    return isinstance(nested, dict) and nested.get("scoring_version") == "v2"


def _samples_to_payload(rows: list[SensorSample]) -> list[dict]:
    """Mirror TripProcessingService._samples_to_payload (speed_mps -> km/h for the pipeline)."""
    payload: list[dict] = []
    for row in rows:
        ts = row.ts
...
def _count_stored_generated_events(breakdown_raw: str | None) -> int:
    if not breakdown_raw:
        return 0
    try:
        breakdown = json.loads(breakdown_raw)
        generated = breakdown.get("generated_events")
        return len(generated) if isinstance(generated, list) else 0
    except Exception:
        return 0


def _load_trip_samples(db, trip_id: str) -> list[SensorSample]:
    return list(
        db.execute(
            select(SensorSample)
            .where(SensorSample.trip_id == trip_id)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Recalculate historical trips with v2 scoring")
    parser.add_argument("--trip-id", type=str, default=None, help="Recalculate only this trip")
    ...
    parser.add_argument("--include-v2", action="store_true", help="Re-run trips already scored under v2")
    parser.add_argument("--limit", type=int, default=None, help="Max trips to process (for testing)")
    ...
    report: dict[str, Any] = {
        ...
        "trip_deltas": [],
        "failures": [],
        "skipped": {"raw_deleted": [], "already_v2": [], "active": []},
    ...
    try:
        ...
        for trip in trips:
            ...
            if not args.include_v2 and _is_scoring_version_v2(trip.score_breakdown):
                skipped_v2 += 1
                report["skipped"]["already_v2"].append(trip.id)
                continue

            old_score = trip.score
            old_events = _count_stored_generated_events(trip.score_breakdown)

            if args.dry_run:
                samples = _load_trip_samples(db, trip.id)
        ...
        if score_after:
            ...
            totals["score_unchanged_count"] = sum(1 for d in score_deltas if d == 0)
